...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { postId, tableName = "z_insta_sche_kobitar_sohor" } = body;
    let posts = [];

    // Determine the status column name based on the table
    const statusColumn = tableName === "z_insta_post_db" ? "status_kobitar_sohor" : "status";
    const publishedAtColumn = tableName === "z_insta_post_db" ? "published_at_kobitar_sohor" : "published_at";
